	...
	<div>
		...
		>
			<div v-if="loading" style="display: flex; align-items: center;">
				<div class="loading" />
				<div v-if="loadingText" style="margin-left: 10px;">
					<slot>{{ loadingText }}</slot>
				</div>
			</div>

			<div class="" v-if="icon" style="display: flex; align-items: center;">
				<my-icon :icon="icon" :color="iconColor"/>
				<div style="margin-left: 10px;">
					<slot>{{ text }}</slot>
				</div>
				<!-- <slot><div style="margin-left: 10px;">{{ text }}</div></slot> -->
			</div>

			<div class="" v-if="!icon && !loading">
				<slot>{{ text }}</slot>
			</div>
		</div>
	</div>
