	...
	<div class="container">
		...
		<el-card class="box-card">
			...
			<el-table :data="tableData" style="width: 100%" @selection-change="selectable">
				...
				<el-table-column prop="title" label="被举报文章" align="center" show-overflow-tooltip />
				<el-table-column label="举报类型" width="70px">
					<template #default="scope">
						<el-tag size="medium" v-if="scope.row.type == 0">其他问题 </el-tag>
						<el-tag size="medium" v-else-if="scope.row.type == 1">标题夸张</el-tag>
						<el-tag size="medium" v-else-if="scope.row.type == 2">低俗色情</el-tag>
						<el-tag size="medium" v-else-if="scope.row.type == 3">错别字多</el-tag>
						<el-tag size="medium" v-else-if="scope.row.type == 4">旧闻重复</el-tag>
						<el-tag size="medium" v-else-if="scope.row.type == 5">涉嫌广告</el-tag>
						<el-tag size="medium" v-else-if="scope.row.type == 6">内容抄袭</el-tag>
						<el-tag size="medium" v-else-if="scope.row.type == 7">政治相关</el-tag>
						<el-tag size="medium" v-else-if="scope.row.type == 8">内容不实</el-tag>
						<el-tag size="medium" v-else-if="scope.row.type == 9">涉嫌违法犯罪</el-tag>
						<el-tag size="medium" v-else-if="scope.row.type == 10">侵犯名誉/隐私/著作/肖像权等</el-tag>
					</template>
				</el-table-column>
				<el-table-column prop="user_name" label="举报人" align="center" show-overflow-tooltip />
				<el-table-column prop="remark" label="举报描述" align="center" show-overflow-tooltip />
				<el-table-column prop="status" label="处理状态">
					<template #default="scope">
						<!-- 0:审核失败;1:审核中;2:审核完成,内容未违规,3:审核完成,内容违规已删除该文章 -->
						<p v-if="scope.row.status == 0">审核失败</p>
						<p v-else-if="scope.row.status == 1">审核中 </p>
						<p v-else-if="scope.row.status == 2">审核完成,内容未违规 </p>
						<p v-else-if="scope.row.status == 3">审核完成,内容违规已删除该文章 </p>
					</template>
				</el-table-column>
				<el-table-column prop="createTime" label="举报时间" align="center" width="140" />
		...
		<el-dialog v-model="dialogVisible" :title="dialogTitle" draggable @close="closeDialog">
			<el-form ref="dictFormRef" :model="Form" :rules="Rules" label-width="80px" status-icon>
				<el-radio-group v-model="Form.status">
				  ...
				  <el-radio :label="2">审核完成,内容未违规</el-radio>
				  <el-radio :label="3">审核完成,内容违规已删除该文章</el-radio>
				</el-radio-group>
			</el-form>
			<template #footer>
